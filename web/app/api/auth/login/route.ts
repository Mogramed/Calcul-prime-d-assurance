...
import { loginUpstreamAccount } from "@/lib/server/upstream-auth";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const session = await loginUpstreamAccount(payload, clientSession.clientId);
    ...
    return clientSession.attach(errorJsonResponse(error, "La connexion est temporairement indisponible."));
  }
}
